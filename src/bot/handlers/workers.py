...
async def help_worker(message: Message):
    await message.answer(
        "Для реєстрації введіть команду:\n `/registration`", parse_mode="Markdown"
    )
...
async def save_data(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await SetMyCommands(
        commands=worker_commands, scope=BotCommandScopeChat(chat_id=user_id)
    )
    await message.answer(
        "Ви додані в базу робітників, тепер Ви маєте змогу брати заявки"
    )


@worker_router.message(
    Command("complete"), ChatTypeFilter(chat_type=["private"]), WorkerFilter()
)
async def show_all_applications(
    ...
    application_query = (
        ...
        .where(
            and_(
                Worker.user_id == str(message.from_user.id),
    ...
    applications = [
        (application.address, application.id)
        for application in application_request.scalars()
    ]

    if len(applications) > 0:
        await message.answer(
            "Виберіть замовлення:", reply_markup=show_applications(applications)
        )
...
@worker_router.callback_query(
    Text(startswith="chose_application"), CompleteStates.application_id
)
...
async def chose_application(message: Message, state: FSMContext, session: AsyncSession):
    ...
    media_group = [
        InputMediaPhoto(
            ...
            caption=f'Акт - {data["application_id"]}\nCума:{data["price"]}',
        )
    ]
    ...
    application_query = (
        ...
        .where(ApplicationWorkerAssociation.application_id == data["application_id"])
        .values(status="Ready")
    )
    ...
    application_query = (
        ...
        .values(
            complete_time=message.date.replace(tzinfo=None),
            price=data["price"],
...
@worker_router.message(
    Command("cancel"), ChatTypeFilter(chat_type=["private"]), WorkerFilter()
)
async def show_all_applications(
    message: Message, state: FSMContext, session: AsyncSession
):
...
@worker_router.callback_query(
    Text(startswith="chose_application"), CancelStates.application_id
)
async def chose_application(callback: CallbackQuery, state: FSMContext):
    await state.update_data(application_id=int(callback.data.split("_")[2]))
