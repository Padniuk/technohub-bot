...
async def help(message: Message):
    await message.answer(
        "Для створення посту введіть команду:\n `/post`", parse_mode="Markdown"
    )
...
async def make_post(message: Message):
    await message.answer(
        "Оберіть спосіб подачі заявки", reply_markup=choose_form_type()
    )
...
async def fill_answers(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Оберіть тип послуг:", reply_markup=choose_service_type()
    )
...
@user_router.callback_query(
    Text(startswith="send_customer_info"), WorkerFilter(is_block_check=True)
)
async def show_contacts(callback: CallbackQuery, session: AsyncSession):
    ...
    application_select = select(Application).where(
        Application.message_id == str(callback.message.message_id)
    )
    ...
    if len(existing_status) > 0:
        ...
        association = ApplicationWorkerAssociation(
            application_id=application.id, worker_id=worker.id, status="Up", comment=""
        )
